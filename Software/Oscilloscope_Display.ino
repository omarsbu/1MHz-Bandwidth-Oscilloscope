...
// LMH6881 SPI pins
#define LMH6881_SPI_CS 22 
#define LMH6881_SPI_CLK 1 // pin is MUX select unless gain is being changed 
#define LMH6881_SPI_SDA 3 // pin is MUX select unless gain is being changed
#define LMH6881_SPI_SDI 21 // pin is MUX select unless gain is being changed

// Display control pushbutton MUX select pins
#define PB_mux_s0 1
#define PB_mux_s1 3
#define PB_mux_s2 22

// Serial data regarding push button press
#define PB_input 5  // MUX inputs are read serially by counting through select addresses

/* Create LUT to map MUX input to PB presses */

// Trigger signals
#define trigger_comparator 17 // compares input signal to trigger level
#define trigger_DAC 16  // outputs analog trigger voltage to comparator input
...
double volts_per_div = 2.5;  // Volts per vertical division
...
// Plotting parameters
double trigger_level = 0; // tigger voltage
double ADCsamplebuffer[250]; // samples taken from ADC, 1 for each pixel on x-axis 
...
void loop() {
  ...
  do {
    ADCsamplebuffer[0] = analogRead(ADC_input);
  }
  while (ADCsamplebuffer[0] != trigger_level)
